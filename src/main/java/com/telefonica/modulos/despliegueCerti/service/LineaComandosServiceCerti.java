...
public class LineaComandosServiceCerti {

    private Session session = null;
    private ChannelSftp channelSftp = null;
    private ChannelExec channelExec = null;
    private final LineaComandosBean lineaComandosBean;
    private final PanelConsolaCerti panelConsolaCerti;

    public LineaComandosServiceCerti(LineaComandosBean lineaComandosBean, PanelConsolaCerti panelConsolaCerti) {
        this.lineaComandosBean = lineaComandosBean;
        this.panelConsolaCerti = panelConsolaCerti;
    }
    ...
    public void moverFicherosAMaquina(List<String> listaActivos, String directorioLocalFicheros) throws Exception {
        try {
            ...
            for (String activoAux : listaActivos) {
                ...
                panelConsolaCerti.addText("Fichero añadido: " + lineaComandosBean.getDirectorioConstruccion() + "/" + activo + "\n");
            }
        } catch (Exception e) {
                ...
                if (!folder.isEmpty()) {
                    try {
                        ...
                        if(directorio.toString().contains("PRUEBA_DESPLIEGUE_AUTOMATIZADO")){
                            ...
                            panelConsolaCerti.addText("Directorio creado: " + directorio + "\n");
                        }
                    }
                }
            }
    ...
    private void moverWarCarpetaLocal(String directorioLocalFicheros) throws Exception {
        try {
            panelConsolaCerti.addText("Moviendo war del portal de la máquina a la carpeta local...\n");
            crearConexionSFTP(lineaComandosBean.getMaquinaTramitador().getHost(), lineaComandosBean.getMaquinaTramitador().getUser(),
                    lineaComandosBean.getMaquinaTramitador().getPassword());
            ...
            panelConsolaCerti.addText("Se ha movido el war del portal de la máquina a la carpeta local: " + directorioLocalFicheros + "\n");
        } catch (Exception e) {
            panelConsolaCerti.addText("ERROR - " + e.getMessage() + "\n");
            throw e;
        } finally {
    ...
    private void moverWarPortalAMaquina(String directorioLocalFicheros) throws Exception {
        try {
            ...
            panelConsolaCerti.addText("Renombrando war existente como backup...\n");
            ...
            panelConsolaCerti.addText("Moviendo war del portal de la carpeta local a la máquina...\n");
            channelSftp.put(rutaLocalWar, lineaComandosBean.getMaquinaPortal().getDirectorioDejarWar());
            panelConsolaCerti.addText("Se ha movido el war del portal de la carpeta local a la máquina: " + lineaComandosBean.getMaquinaPortal().getDirectorioDejarWar() + "\n");
        } catch (Exception e) {
            panelConsolaCerti.addText("ERROR - " + e.getMessage() + "\n");
            throw e;
        } finally {
    ...
    public void desplegarYReiniciar(boolean desplegarSSNN, boolean desplegarPortal) throws Exception {
        if(desplegarSSNN){
            panelConsolaCerti.addText("Desplegando ficheros SSN/fases/tramitador y reiniciando servidores...\n");
            // desplegar
            ...
            panelConsolaCerti.addText("Ficheros SSN/fases/tramitador desplegados y servidores reiniciados\n");
        }
        if(desplegarPortal){
            panelConsolaCerti.addText("Desplegando ficheros portal y reiniciando servidores...\n");
            // desplegar
            ...
            panelConsolaCerti.addText("Ficheros portal desplegados y servidores reiniciados\n");
        }
    }
    ...
    private void pintarTrazas() throws IOException, InterruptedException {
        ...
        while(true){
            while(in.available()>0){
                ...
                panelConsolaCerti.addText(text);
            }
            if(channelExec.isClosed()){
                ...
                if (error.available() > 0) {
                    ...
                    panelConsolaCerti.addText(errorText);
                }
