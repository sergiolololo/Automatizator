...
public class PanelHistoricoLogsCerti extends JDialog {
	...
	public PanelHistoricoLogsCerti(ApplicationContext appContext, String rutaCarpetaLogs) {
		...
		tablaLogs.addMouseListener(new MouseAdapter() {
			public void mousePressed(MouseEvent mouseEvent) {
				...
				if (mouseEvent.getClickCount() == 2 && table.getSelectedRow() != -1) {
					PanelConsolaCerti panelConsolaCerti = new PanelConsolaCerti(appContext);
					...
					try (Stream<String> stream = Files.lines(Paths.get(rutaArchivo), StandardCharsets.ISO_8859_1)) {
						stream.forEach(line -> panelConsolaCerti.addText(line + "\n"));
					} catch (IOException e) {
						e.printStackTrace();
					}
